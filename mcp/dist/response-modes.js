export const responseModes = ["compact", "standard", "full"];
export function normalizeResponseMode(input) {
    if (input === "standard" || input === "full") {
        return input;
    }
    return "compact";
}
export function wrapResponse(kind, mode, rawPayload, baselineDocumentBytes) {
...
export function estimateJsonPayload(payload) {
...
function shapeCompatibility(mode, payload) {
    ...
    if (mode === "compact") {
        return {
            ...
            required_extensions: asArray(compatibility?.requiredExtensions).map((item) => asRecord(item)?.extension_id ?? item),
