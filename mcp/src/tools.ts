import type { IndexedEntity, IndexedSectionKey, PrdIndexResult } from "./prd-index.js";
...
export const agentPacketPresets = ["implementation", "review", "triage", "schema_change"] as const;
...
export function searchPrd(
  query: string,
  indexResult: PrdIndexResult,
  sections: string[] = [],
  limit = 10
): SearchResult[] {
...
export function getEntityResult(id: string, load: PrdLoadResult, indexResult: PrdIndexResult) {
  return buildEntityPayload(id, load, indexResult);
}

export function getLinkedEntitiesResult(id: string, load: PrdLoadResult, indexResult: PrdIndexResult) {
...
export function listBlockersResult(load: PrdLoadResult, indexResult: PrdIndexResult) {
...
export function listOpenQuestionsResult(load: PrdLoadResult) {
...
export function listProposedDecisionsResult(load: PrdLoadResult) {
  const document = asRecord(load.document);
...
export function buildAgentPacketResult(
  load: PrdLoadResult,
  indexResult: PrdIndexResult,
  ids: string[] = [],
  sections: string[] = [],
...
function appendWithinBudget(packet: AgentPacket, target: unknown[], item: unknown, maxTokens: number): boolean {
  target.push(item);
...
function scoreEntity(query: string, entity: IndexedEntity): SearchResult | null {
...
function scoreSection(query: string, section: string): SearchResult | null {
...
function matchesSectionFilter(section: string | IndexedSectionKey | "meta", filters: string[]): boolean {
  if (filters.length === 0) {
    return true;
  }
